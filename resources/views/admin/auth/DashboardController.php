...
class DashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        /*if($sessionUID!=null){
            ...
            $to_user_ids = Meeting::join('users','users.id','=','meetings.to_user_id','left')->select('meetings.to_user_id','meetings.id','users.role_id')->where('meetings.to_user_id','!=',$user_id)->where('users.role_id','=', 3)->orderBy('meetings.id', 'DESC')->first();
            //echo "<pre>";print_r($to_user_ids);die;

             $meeting_id=  $to_user_ids->id;
           // echo $meeting_id=  $to_user_ids->role;
            ...
            $uni_datas = University::join('meetings','meetings.university_id','=','universities.id','left')->groupBy('meetings.university_id')->get();
            ...
            $meeting_data = Meeting::join('users','users.id','=','meetings.from_user_id','left')->where('meetings.from_user_id',auth::user()->id)->get();
              foreach($meeting_data as $values){
               echo $user_id = $values->to_user_id;echo "<br>";
            ...
            if($role_id == 3){
                ...
                $logged_in_interest = UserInterestAreas::where('status', 1)->where('role_id', $role_id)->where('user_id', $loggedIn_user_id)->get();
                ...
                $matched_alumni = UserInterestAreas::join('users','users.id','=','user_interest_areas.user_id','left')->whereIn('interest_id', $loggedin_user_interest_data)->where('user_interest_areas.status',1)->where('users.university_id',$university_id)->where('user_interest_areas.role_id',4)->groupBy('user_interest_areas.user_id')->get();
            //echo "<pre>";print_r($matched_alumni);die;
            }
            return view('admin.dashboard',compact('university_count','graduates_count','alumini_count','booster_count','meeting_count_alum', 'encode','permission_array','users_name','meeting_id','uni_name_array','values_array','matched_alumni'));
    ...
    public function send_email(){
        ...
        foreach($user as $value){
         ...
         $message->to($email,'Pathport')->subject('Survey');
